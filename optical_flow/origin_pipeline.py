...
def _executed_cmds(args, im_pairs, out_dire, dm_file, dm_options, df_file, df_options, cf_file, cf_options):
	...
	for im_i in xrange(im_n):
		...
		if run_dm:
			df_cmd   = "%s %s %s %s %s -match %s" % (df_file, im_path1, \
										im_path2, flo_path, df_options, mth_path)
		else:
			df_cmd   = "%s %s %s %s %s" % (df_file, im_path1, im_path2, \
										flo_path, df_options)
		...
		if im_disp:
			...
			im3 = cv2.imread(of_path)
			cv2.imshow(im_name2, im1)
			cv2.waitKey(0)
			cv2.destroyAllWindows()
			cv2.imshow(im_name2, im2)
			cv2.waitKey(0)
			cv2.destroyAllWindows()
			cv2.imshow(im_name2, im3)
			cv2.waitKey(0)
			cv2.destroyAllWindows()
